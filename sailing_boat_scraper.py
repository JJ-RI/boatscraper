...
Monitors dba.dk, blocket.se, finn.no, ebay-kleinanzeigen.de, and marktplaats.nl
...
class SailingBoatScraper:
    ...
    def scrape_blocket_se(self):
        ...
        try:
            ...
            # Blocket uses dynamic loading, so we'll get what we can from initial HTML
            ...
            for listing in listings[:20]:
                try:
                    ...
                    price_elem = listing.find(text=re.compile(r'kr'))
    ...
    def scrape_kleinanzeigen_de(self):
        """Scrape sailing boats from ebay-kleinanzeigen.de (now kleinanzeigen.de)"""
    ...
    def scrape_marktplaats_nl(self):
        ...
        try:
            ...
            # Marktplaats structure may vary
    ...
    def scrape_all(self):
        ...
        time.sleep(1)  # Reduced delay for GitHub Actions
    ...
    def generate_rss(self):
        ...
        # Get boats from last 7 days
        cutoff_date = datetime.now() - timedelta(days=7)
        
        recent_boats = [
            boat for boat in self.boats.values()
            if datetime.fromisoformat(boat['date_found']) > cutoff_date
        ]
        
        # Sort by date found (newest first)
        recent_boats.sort(key=lambda x: x['date_found'], reverse=True)
        ...
        for boat in recent_boats:
            ...
            fe.published(datetime.fromisoformat(boat['date_found']))
...
def main():
    ...
    if new_boats:
        ...
        for i, boat in enumerate(new_boats[:15], 1):  # Show first 15
    ...
    rss_file = scraper.generate_rss()
    
    print(f"\n✨ Scraper completed successfully!")
    print(f"Run finished: {datetime.now().strftime('%Y-%m-%d %H:%M:%S UTC')}")
